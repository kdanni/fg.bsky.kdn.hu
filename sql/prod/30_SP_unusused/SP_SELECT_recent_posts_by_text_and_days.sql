...
DROP PROCEDURE IF EXISTS sp_SELECT_recent_posts_by_text_and_days;

--   author_did VARCHAR(255) NOT NULL,
CREATE PROCEDURE sp_SELECT_recent_posts_by_text_and_days ( 
    p_text TEXT,
    p_days INT
)
BEGIN

    -- recent posts are created in the last p_days days
    SET @days_ago = DATE_SUB(CURDATE(), INTERVAL p_days DAY);
