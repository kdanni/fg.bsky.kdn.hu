...
export async function backfillFavorites(actor = BACKFILL_ACTOR) {
    try {
        ...
        while (cursor !== undefined && loop < LOOP_LIMIT) {
            ...
            const url = `${BSKY_PUBLIC_API_ROOT}/xrpc/app.bsky.feed.getActorLikes?actor=${actor}${cursorParam}`;
            ...
            if (response && response.feed) {
                ...
                for (const item of (response.feed || [])) {
                    ...
                    const itemExists = await pool.query(
                        'SELECT furl FROM favorites_post WHERE url = ? ', [item?.post?.uri||null]);
                    ...
                    const sql = 'CALL upsert_favorite(?, ?, ?, ?, ?)';
                    const params = [
                        item?.post?.uri||null,
                        has_image||null,
                        safeForWorkScore||-1,
